...
// messageSendAtText('21341182572@chatroom', '@超哥 hello world', ['ledongmao', 'wxid_pnza7m7kf9tq12'])
// messageSendAtText('21341182572@chatroom', '@超哥 hello world', ['ledongmao'])
...
// sendImageMsg('ledongmao', 'C:\\Users\\tyutl\\Documents\\WeChat Files\\wxid_0o1t51l3f57221\\FileStorage\\MsgAttach\\01c2dbb9bb49519d3708de94a13d0d42\\Thumb\\2024-06\\message-7536055685103019865-url-thumb.jpg')
...
Interceptor.attach(
    ...
    onEnter(args) {
        try {
            // 参数打印
            console.log("sendPatMsg called with args: " + args[0] + ", " + args[1]);
            
            if (!args[0].isNull()) {
                console.log('sendPatMsg roomId:', readWStringPtr(args[0]).readUtf16String());
            }
            if (!args[1].isNull()) {
                console.log('sendPatMsg contactId:', readWStringPtr(args[1]).readUtf16String());
            }
...
// 获取数据库句柄
function getDbHandle(base: NativePointer, offset: number) {
...
// 获取消息数据库句柄
function getMsgDbHandle(msgMgrAddr: NativePointer) {
...
// 获取所有数据库句柄
function getDbHandles(): Map<string, NativePointer> {
    dbMap.clear();
...
// 获取数据库表结构
function getDbTables(db: string): Array<{ name: string, sql: string }> {
...
// 执行 SQL 查询
function execDbQuery(db: string, sql: string): Array<{ [key: string]: Uint8Array | string }> {
...
const sql = 'select UserName,Alias,NickName,Remark,LabelIDList,DomainList,ChatRoomType,BigHeadImgUrl,SmallHeadImgUrl,ChatRoomNotify from Contact where NickName!="" limit 100;'

// console.log('execDbQuery() res:\n', JSON.stringify(execDbQuery('MicroMsg.db', sql), null, 2))

// 获取本地ID和数据库索引
function getLocalIdAndDbIdx(id: number): { localId: number, dbIdx: number } | null {
...
// 获取音频数据
function getAudioData(id: number): Uint8Array | null {
    const msgMgrAddr = moduleBaseAddress.add(OFFSET_DB_MSG_MGR).readPointer();
    const dbIndex = msgMgrAddr.add(0x68).readU32();
...
// 发送文本消息
function sendTextMessage(contactId: string, text: string, atWxids?: string[]): number {
    const send_message_mgr_addr = moduleBaseAddress.add(offsets.kSendMessageMgr);
    const send_text_msg_addr = moduleBaseAddress.add(offsets.kSendTextMsg);
    const free_chat_msg_addr = moduleBaseAddress.add(offsets.kFreeChatMsg);
    ...
    const text_msg = writeWStringPtr(text);

    let temp: NativePointer;
    if (atWxids && atWxids.length > 0) {
        const tempSize = 0x20 * atWxids.length;
        temp = Memory.alloc(tempSize);
        for (let i = 0; i < atWxids.length; i++) {
            const wxidWStringPtr = writeWStringPtr(atWxids[i]);
            temp.add(0x20 * i).writePointer(wxidWStringPtr);
        }
    } else {
        temp = Memory.alloc(0x20);
        temp.writePointer(ptr(0));
    }
    ...
    mgr();
    const success = send(chat_msg, to_user, text_msg, temp, 1, 1, 0, 0);
    free(chat_msg);
...
// 发送图片消息
function sendImageMessage(contactId: string, path: string): number {
...
// 发送文件消息
function sendFileMessage(contactId: string, path: string): number {
    const new_chat_msg_addr = moduleBaseAddress.add(offsets.kNewChatMsg);
    const free_chat_msg_addr = moduleBaseAddress.add(offsets.kFreeChatMsg);
...
// 发送拍一拍消息
function sendPatMessage(roomId: string, contactId: string): number {
    const send_pat_msg_addr = moduleBaseAddress.add(offsets.kSendPatMsg);

    const roomIdStr = writeWStringPtr(roomId);
    const contactIdStr = writeWStringPtr(contactId);

    const send = new NativeFunction(send_pat_msg_addr, 'int64', ['pointer', 'pointer', 'int64']);

    const arg3 = Memory.alloc(0x8);
    arg3.writeU64(0x0);

    const success = send(roomIdStr, contactIdStr, 0);

    return Number(success) > 0 ? 1 : 0;
}
console.log('sendPatMessage() res:\n', sendPatMessage('21341182572@chatroom', 'ledongmao'))

// 转发消息
function forwardMessage(msgId: number, receiver: string): number {
