...
var getContactNativeFunction = (function () {
    ...
    function recurse(node, contactList, nodeList, headerNodeAddress) {
        ...
        for (var _i = 0, nodeList_1 = nodeList; _i < nodeList_1.length; _i++) {
            var item = nodeList_1[_i];
            if (node.equals(item)) {
                return;
            }
        }
        nodeList.push(node);
        var wxid = readWideString(node.add(0x30));
        var name = readWideString(node.add(0x8c));
        var contactJson = {
            id: wxid,
            name: name,
        };
    ...
    // 获取头结点地址
    var baseAddress = moduleBaseAddress.add(offset.contactInfo.nodeOffset).readPointer();
    if (baseAddress.isNull()) {
        return '[]';
    }
    var headerNodeAddress = baseAddress.add(offset.contactInfo.nodeRootOffset).readPointer();
    var contactList = [];
    var nodeList = [];
...
var getChatroomMemberNickInfoFunction = (function (memberId, roomId) {
    ...
    var nickRoomId = null;
    var nickMemberId = null;
    var nickBuff = Memory.alloc(0x7e4);
    //const nickRetAddr = Memory.alloc(0x04)
    memberNickBuffAsm = Memory.alloc(Process.pageSize);
    //console.log('asm address----------',memberNickBuffAsm)
    nickRoomId = initidStruct(roomId);
